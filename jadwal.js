...
function displayMatchInfo(matches) {
    ...
    matches.forEach(match => {
        ...
        const matchJson = {
            ...
            start_time: `${matchDate}T${formattedTime}:00`,
            duration: 110, // Placeholder untuk durasi
            thumbnail: "00000", // Placeholder untuk thumbnail
            watch_url: "00000" // Placeholder untuk watch_url
        };
...
`;
    ...
    document.getElementById('copyTextButton').onclick = () => copyMatchInfo(allMatchJson.map(match => `${match.title}\nTanggal: ${match.start_time.split('T')[0]}\nWaktu: ${match.start_time.split('T')[1]}\nKompetisi: ${match.league}\nLink: https://bit.ly/zonasportlive`).join('\n\n'));
