...
async function loadMatches() {
    try {
        ...
        const cacheTime = sessionStorage.getItem('matchesTime');
        ...
        if (cache && cacheTime && (now - cacheTime < cacheDuration)) {
            matches = JSON.parse(cache);
        } else {
            matches = await fetchMatches();
            sessionStorage.setItem('matches', JSON.stringify(matches));
            sessionStorage.setItem('matchesTime', now);
        }
...
async function fetchMatches() {
    ...
    matches.forEach(match => {
        match.liveUrl = liveUrls[match.videoId] || match.m3u8Url;
    });
...
async function fetchLiveUrls(matches) {
    ...
    const cacheKey = `liveUrls-${videoIds.join(',')}`;
    ...
    localStorage.setItem(cacheKey, JSON.stringify(liveUrls));
    return liveUrls;
}
...
function renderMatches(matches) {
    ...
    matches.forEach(match => {
        const streamUrl = match.liveUrl || match.m3u8Url;
        const now = new Date();
        ...
        card.innerHTML = `
                    ...
                    <button class="watch-button" onclick="playMatch('${match.videoId}', '${match.liveUrl}')">
...
// Mulai JW Player
function playMatch(videoId, streamUrl) {
    jwplayer("jwplayerContainer").setup({
        file: streamUrl,
        width: "100%",
        aspectratio: "16:9",
        image: "https://da.gd/EtS3Gy"
    });
    ...
    // Muat dan tampilkan daftar channel
    loadChannels(videoId);
}

// Fungsi untuk memuat dan menampilkan daftar channel
...
// Memuat data baru setiap 5 menit
