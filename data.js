// Token Bot Telegram dan Chat ID Channel
const telegramBotToken = '7598004626:AAGG4SAfiiZ76AHaEWozvc2_I5WDk23795c';
const telegramChatId = '1391979650';
...
async function fetchMatches() {
    const response = await fetch('https://wartakita.github.io/bola.github.io/schedule.json');
...
function renderMatches(matches) {
    ...
    matches.forEach(match => {
        ...
        // Kirim notifikasi ke Telegram jika pertandingan sedang berlangsung dan belum dikirim notifikasi sebelumnya
        if (isLive && !sentNotifications.has(match.videoId)) {
            sendTelegramNotification(match);
            sentNotifications.add(match.videoId);
        }
    });
}
...
function playMatch(videoId, liveUrl, m3u8Url, iframeURL, matchTitle) {
    ...
    fetch(streamUrl)
        .then(response => {
            if (!response.ok) {
                throw new Error('Stream URL failed');
            }
            return response;
        })
        .catch(error => {
        ...
        .finally(() => {
            if (streamUrl.endsWith('.m3u8')) {
                ...
                document.getElementById('jwplayerContainer').innerHTML = `<iframe src="${streamUrl}" width="100%" height="100%" scrolling="no" sandbox="allow-scripts allow-same-origin" allowfullscreen="true"></iframe>`;
            }
...
function renderChannels(channels) {
    ...
    channels.forEach(channel => {
        ...
        item.innerHTML = `
                    <button onclick="playChannel('${channel.streaming_urls[0]}')">${channel.name}</button>
                `;
...
function renderServerMenu(channels) {
    ...
    channels.forEach((channel, index) => {
        channel.streaming_urls.forEach((streamUrl, streamIndex) => {
            ...
            button.onclick = () => playChannel(streamUrl);
...
function sendTelegramNotification(match) {
    const message = `🚨 Live Now: ${match.teams} 🚨\nDate: ${formatDate(match.date)}\nTime: ${match.time}\nLeague: ${match.league}\nLink: https://zonasportlive.blogspot.com/`;
