...
async function fetchMatches() {
    ...
    for (const match of matches) {
        try {
            const apiResponse = await fetch(`https://api.90min.to/v1/match/channels?matchId=${match.videoId}`);
            if (apiResponse.ok) {
                const apiData = await apiResponse.json();
                if (apiData.status && apiData.result.length > 0) {
                    match.liveUrl = apiData.result[0].streaming_urls[0];
                }
            }
        } catch (error) {
            console.error(`Failed to fetch live URL for matchId ${match.videoId}:`, error);
        }
    }
...
function renderMatches(matches) {
    ...
    matches.forEach(match => {
        const streamUrl = match.liveUrl || `https://warning.hayo-pencuri-sk21xyz.workers.dev/https://play1nm.hnyongshun.cn/live/ballbar_${match.videoId}.m3u8`;
        const now = new Date();
