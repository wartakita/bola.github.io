...
async function loadMatches() {
    try {
        ...
        const cacheDuration = 24 * 60 * 60 * 1000; // 24 jam
...
async function fetchMatches() {
    const response = await fetch('https://wartakita.github.io/bola.github.io/matches.json');
    ...
    matches.forEach(match => {
        match.liveUrl = liveUrls[match.videoId] || `https://pull.dangaoka.com/live/stream-${match.videoId}_lhd.m3u8`;
    });
...
function renderMatches(matches) {
    ...
    matches.forEach(match => {
        const streamUrl = match.liveUrl || `https://pull.dangaoka.com/live/stream-${match.videoId}_lhd.m3u8`;
        const now = new Date();
